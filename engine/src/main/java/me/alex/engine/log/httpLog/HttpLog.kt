...
object HttpLog {
    ...
    fun printGetRequest(tag: String, request: Request) {
        LOG.httpLog(tag, REQUEST_UP_LINE)
        val urls: List<String> = request.url.toString().split("?");
        //打印URL
        LOG.httpLog(
            tag,
            CENTER_LINE + request.method + " : " + urls[0]
        )
        //打印URL中参数
        if (urls.size > 1) {
            getUrlParameter(tag, urls[1])
        }
        //打印headers
        if (request.headers.size > 0) {
            LOG.httpLog(
                tag,
                CENTER_LINE + HEADERS_TAG + getHeaders(request.headers)
            )
        }
        //打印body
        request.body?.apply {
            LOG.httpLog(tag, CENTER_LINE + BODY_TAG + request.body)
        }
        LOG.httpLog(tag, END_LINE)
    }

    private fun getUrlParameter(tag: String, urls: String) {
        var parStr: String = ""
        val sb = StringBuffer()
        parStr = urls.replace("=", ":").replace("&", ",")
//        parStr = parStr.replace("&", ",")
        sb.append("{").append(parStr).append("}")
        LOG.httpLog(tag, CENTER_LINE + PAR_TAG + sb.toString())
    }

    private fun getHeaders(headers: Headers): String {
        var headerStr = ""
        headers.toMultimap().forEach {
            headerStr += "{ " + it.key + ":" + it.value.toString().replace("[", "")
                .replace("]", " }") + "  "
        }
        return headerStr
    }
    ...
    fun printJsonResponse(
        ...
        LOG.httpLog(tag, DEFAULT_LINE + URL_TAG + responseUrl)
        LOG.httpLog(tag, DEFAULT_LINE + TIME_TAG + chainMs + "ms")
